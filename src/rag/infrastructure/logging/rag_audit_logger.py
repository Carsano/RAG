...
class RAGAuditLogger:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            db_path: Path to the DuckDB database containing audit tables.
            jsonl_path: File path used for the JSONL snapshot log.
        """
    ...
    def _append_jsonl(self, serialized: str) -> None:
        """Append the serialized payload to the JSONL snapshot.

        Args:
            serialized: JSON string representing the payload.
        """
        try:
    ...
    def _persist_duckdb(
        ...
        """Persist payload data inside DuckDB tables.

        Args:
            payload: Raw payload dict.
            serialized: JSON string saved inside the audit table.
        """
        try:
    ...
    def _upsert_user(
        ...
        """Insert the user row if it does not already exist.

        Args:
            con: DuckDB connection.
            user_id: Identifier to insert.
        """
        if not user_id:
            return
        con.execute(
    ...
    def _insert_audit_json(
        ...
        """Insert the raw JSON snapshot row.

        Args:
            con: DuckDB connection.
            request_id: Unique request identifier.
            timestamp: Timestamp associated with the request.
            serialized: Raw JSON document.
            integrity_hash: SHA-256 hash of the JSON payload.
        """
        con.execute(
            """
    ...
    def _insert_request_row(
        ...
        """Insert the flattened request analytics data.

        Args:
            con: DuckDB connection.
            request_id: Identifier for the audit entry.
            timestamp: UTC timestamp string.
            user_id: Optional user identifier.
            components: Mapping of component versions.
            latencies: Latencies collected for the request.
            llm_meta: Token and latency metadata from the LLM.
        """
        con.execute(
            """
            INSERT INTO request (
    ...
    def _insert_metrics(
        ...
        """Insert metric values for the current request.
    ...
    def _insert_chunks(
        ...
        """Insert chunk snapshots generated by retrieval and rerank.

        Args:
            con: DuckDB connection.
            request_id: Identifier for the audit entry.
            rerank_chunks: Reranker outputs to persist.
        """
